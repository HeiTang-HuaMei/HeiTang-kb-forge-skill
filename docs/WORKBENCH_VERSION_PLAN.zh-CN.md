...
当前 Agent / Skill 仍偏静态导出，不能形成真正绑定知识库的专属行业 Agent。
...
单知识库 / 单 Agent 不足，用户需要多知识库、多 Agent、多模型协作。
...
用户可以创建多个知识库；创建多个 Agent；每个 Agent 绑定不同知识库和模型；多个 Agent 可以按 workflow 协作；系统输出协作 trace；Agent 私有记忆默认隔离。
