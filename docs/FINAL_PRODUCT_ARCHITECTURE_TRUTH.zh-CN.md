...
本文档是当前 v4.2.0 Core 状态的短版人工可读真值入口，用于集中说明哪些已实现、哪些部分实现、哪些是未来能力、哪些仍然阻断。机器可读证据保留在 `docs/audits/local_acceptance/large_bilingual_run/` 的历史大文件验收记录、`docs/audits/local_acceptance/pre_v4_p0_after_live_llm/` 的最新 Core P0 after-live-LLM 证明、`docs/audits/p2_1_parser_ocr_backends/` 的 P2.1 Parser/OCR backend release evidence、`docs/audits/p2_2/` 的 P2.2 supplemental version plan，以及 `docs/testing/VALIDATION_GATE_MANIFEST.json` 的 v4.2.0 release validation。
...
- v4.1.1 test governance：validation gate manifest、changed-file impact selector、validation runner、pytest markers 和 obsolete-test pruning register 作为 P2.2 Entry Gate baseline 已存在。
- v4.2.0 P2.2 Skill Suite governance：已有 knowledge package 可以生成 evidence windows、methodology maps、skill candidates、Planning / Functional / Atomic hierarchy、routing rules、dependency graph、validation、diff、installability、governance report 和受控 Skill Pack export。
- 历史 v4 RC readiness evidence 与 v4.1.0 Parser/OCR stable baseline 继续保留，用于 audit continuity。
...
| Agent 与 Skill | Legacy Skill package、standalone Agent、KB-bound Agent、本地确定性 runtime smoke、KB boundary、mother/child contracts、memory policy reports 和 P2.2 Skill Suite generation 已存在。P2.2 路径生成 methodology-backed Planning / Functional / Atomic Skills，并覆盖 routing、dependency graph、validation、diff、installability、governance 和 Skill Pack export。完整 autonomous tool-calling Agent Runtime 未实现。 | implemented with suite governance boundaries |
...
| Test governance | Validation gate manifest、changed-file impact selector、dry-run / executable validation runner、pytest markers、obsolete-test pruning register 和 token-efficient log policy 已存在。 | v4.1.1 baseline，作为 release process 复用 |
| UI | Core 输出 Workbench contracts。P1-RWF-V2 evidence 与 UI consumption 是历史 v4 readiness evidence；P2.1 Workbench sync 可展示 backend matrix、evidence、install mode 与 limitations，但不展示 runtime execution controls；P2.2 UI industrial closure 以 static evidence workflow 展示 Knowledge Package -> Evidence -> Methodology -> Candidates -> Hierarchy -> Skill Suite -> Reports -> Export。 | v4.2.0 UI/CLI industrial closure |
