...
7. Campaign 4 范围只包含 UI Workbench，不包含 Campaign 5 Bridge、Campaign 6 Runtime / Memory、Campaign 9 EXE。
...
| 导航数量 | 顶层导航数量和分组 | 收敛到不超过 7 个入口的方案 |
...
- 顶层导航收敛到不超过 7 个入口。
- task-card driven UI。
- 文件导入、parsing、knowledge splitting、Skill generation、Agent package generation、validation 的进度展示。
- pending、running、completed、failed、retryable、cancelled、blocked 状态。
- 输入区、任务进度区、输出结果区、证据/报告区、错误与重试区。
- UI 不宣称底层 runtime 已完成。
