...
| --- | --- | --- | --- | --- |
| Campaign 4 | UI inventory、七入口 IA 固定为工作台 / 知识库 / 文档 / Skill / Agent / 验证与导出 / 设置与模板、task-card flow 方案、UI dirty diff 审查 | UI focused tests、状态与进度条验证、知识库完整旅程、文档生成完整旅程、Skill 完整旅程、Agent 当前允许范围的定义/绑定/预览/package-export 旅程、无 runtime overclaim | UI handoff，不宣称 Bridge/Provider Runtime/Agent Runtime/EXE complete，不实现或暗示 Campaign 6 Agent Foundation | 截图、静态按钮页、计划文档、Agent package spec alone、Campaign 6 Agent Foundation 功能 |
...
| Provider Runtime Gate | Campaign 5 handoff accepted、provider policy scope 明确、网络同意和 secret boundary 明确 | real external LLM E2E live smoke、provider/model stage selection runtime tests、redaction tests、cost guard tests、failure-mode tests、network consent tests、no secret persistence tests | Provider Runtime handoff，允许 Campaign 6 使用已验收 provider assumptions；不宣称 Agent Runtime complete | mock/offline harness、配置文件、Campaign 7 profile loading、单个 skipped smoke |
| Campaign 6 | Campaign 4 Page-Level UI Redesign owner visual acceptance、Campaign 4+5 UI-Bridge Realignment Gate accepted、Campaign 5 handoff accepted、Provider Runtime Gate accepted、simple/advanced field matrix frozen、Agent schema frozen、visible action capability classification frozen | Agent create/edit/copy/version tests、simple/advanced mode tests、KB and multi-Skill binding tests、basic model/tool/permission/work-partition config tests、validation/save/preview/export Agent package tests、soft deletion/archive tests、no runtime overclaim | Agent Foundation handoff，不宣称 Memory Runtime、Teams/Subagent、Sandbox、A2A 或 EXE complete | Agent package spec alone cannot substitute for Agent Foundation acceptance、未来功能展示页、不可逆物理删除 |
| Campaign 7 | Campaign 6 handoff accepted、profile lifecycle scope 明确、Provider Runtime Gate 已完成且不得在 Campaign 7 首次实现真实 Provider Runtime、不得新增 Agent 核心字段 | profile persistence tests、source precedence tests、load/merge tests、migration/backward compatibility tests、secret-safe injection tests、diagnostics tests、runtime availability check tests、reusable profile tests | Config engineering handoff，不宣称 Full Review complete | 首次加入模型/工具/权限/工作分区/模式核心字段、首次实现真实 Provider Runtime、硬编码配置 |
