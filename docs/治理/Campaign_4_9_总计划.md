...
| Provider Runtime Gate | Provider Runtime 独立前置 | 必须在 Campaign 6 前完成；不塞进 Campaign 7；覆盖真实外部 LLM E2E、provider/model stage selection、live smoke、redaction、cost、failure、secret boundary |
| Campaign 7 | Configuration System Engineering | 只工程化 Campaign 6 已定义配置和已验收 provider/runtime 边界；不得首次加入模型、工具、权限、工作分区、模式核心字段或真实 Provider Runtime |
...
3. Provider Runtime Gate：Campaign 6 前独立完成。
4. Campaign 6：Agent Foundation。
5. Campaign 7：Configuration System Engineering。
6. Campaign 8：Full Testing / Full Review。
7. Campaign 9：EXE Packaging。
8. Final Release：Campaign 9 acceptance 后才允许开始。
...
3. Provider Runtime Gate accepted，且不把真实 Provider Runtime 塞进 Campaign 7。
4. Campaign 6 Entry Gate 冻结 simple-mode field matrix、advanced-mode field matrix、Agent schema。
5. Campaign 6 Entry Gate 为每个可见动作冻结 capability classification：`enabled_real`、`disabled_boundary`、`display_only` 或 `omitted`。
...
Provider Runtime Gate 在 Campaign 6 前独立完成：真实外部 LLM API E2E、provider/model stage selection runtime、live-smoke acceptance、redaction、cost guard、failure handling、network consent、secret boundary。Campaign 7 不得首次实现或吸收真实 Provider Runtime。

Campaign 7 只承接配置系统工程化：Profile 文件化、配置来源优先级、加载与合并、迁移和向后兼容、Secret 安全注入、配置诊断、Runtime availability check、批量复用 Agent Profile。Campaign 7 不得首次加入模型配置、工具配置、权限配置、工作分区、简易模式、复杂模式核心字段或真实 Provider Runtime。

工作分区边界：

- Campaign 6：工作分区模型、权限声明、路径绑定。
- Provider Runtime Gate：真实 provider runtime、stage selection runtime、live smoke acceptance、secret/network/cost/failure gates。
...
Agent Teams、Subagent 策略、Memory Runtime、Compaction、Model Router runtime、Computer Use、Sandbox、A2A、多人协作 runtime 和真实运行隔离均后置到 Post-9 或独立 future gate。当前 UI 中未完成能力必须隐藏或禁用，优先 `omitted`，不得堆叠成一页不可用功能，也不得伪装可用。
...
- 当前导航数量不符合 Campaign 4 固定七入口定义，Campaign 4 顶层导航必须且仅为：工作台 / 知识库 / 文档 / Skill / Agent / 验证与导出 / 设置与模板。

Campaign 4 不得宣称 Bridge complete、Provider Runtime complete、Agent Runtime complete、Memory Runtime complete、Multi-Agent executable complete、EXE packaging complete 或 GitHub Release complete。
