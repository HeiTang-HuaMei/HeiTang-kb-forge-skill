...
| --- | --- | --- | --- |
| Main UI | `web/workbench/flutter_app/lib/main.dart` | About 3,500 lines after the top bar, status bar, sidebar, app shell, and shared layout extractions. Routing, page-surface dispatch, and several product widgets still live together. | Split one UI page, shell widget, or shared widget group at a time into `lib/features/<page>/...`, `lib/app/...`, or `lib/shared/...` without changing visible behavior. |
