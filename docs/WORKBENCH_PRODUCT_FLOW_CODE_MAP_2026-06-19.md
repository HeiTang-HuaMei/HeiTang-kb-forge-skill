...
| --- | --- | --- | --- |
| Main UI | `web/workbench/flutter_app/lib/main.dart` | About 2,770 lines after the top bar, status bar, sidebar, app shell, shared layout, and shared product component extractions. Routing, page-surface dispatch, and artifact action helpers still live together. | Split one UI page, shell widget, or shared widget group at a time into `lib/features/<page>/...`, `lib/app/...`, or `lib/shared/...` without changing visible behavior. |
...
The Shared Product Components slice moves reusable product headers, page tabs, panels, tables, field rows, section captions, capability status markers, settings health labels, state pills, and generic product action buttons into `web/workbench/flutter_app/lib/shared/product_components.dart` with no behavior change. `main.dart` is now about 2,772 lines.
