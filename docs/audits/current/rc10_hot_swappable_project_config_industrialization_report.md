...
- When a probe succeeds, `provider_adapter_readiness_report.json` marks that Provider as `连接成功` and `ready_for_user_selection=true`; explicit activation binds both `retrieval_provider` and `governance_audit_provider` to `ragas` or `deepeval` through `provider_capability_selection_state.json`, so Retrieval Verification and Audit Center see the same quality-gate capability enhancement. Rollback removes both explicit selections, suppresses auto-reselection for both capability IDs, and returns both entries to local fallback.
...
- Runtime status sync is verified for both success and fallback paths:
