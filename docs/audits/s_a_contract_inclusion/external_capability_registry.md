# S/A External Capability Registry

This is contract inclusion only. It does not implement external project functionality, add dependencies, call APIs, or bundle external runtimes.

## Summary

- S projects: 7
- A projects: 16
- Internal capability anchors: 8
- External features implemented: false
- Planned adapters marked ready: false
- Provider/network/API ready: false
- v4.0 started: false

## Projects

| Project | Rating | Contract status | Blocked reason | Post-v4 target |
| --- | --- | --- | --- | --- |
| LLM Wiki v2 | S | future_adapter, capability_anchor | external_project_registry_only | P2.4 |
| WeKnora | S | future_adapter, capability_anchor | external_project_registry_only | P2.5 |
| n8n | S | future_adapter, provider_required, workflow_export | external_project_registry_only | P2.2 / P3 |
| andrej-karpathy-skills | S | benchmark_only, capability_anchor | external_project_registry_only | P2.9 |
| PaddleOCR | S | planned_adapter | external_project_registry_only | P2.6 |
| MinerU | S | planned_adapter | external_project_registry_only | P2.6 |
| Docling | S | planned_adapter | external_project_registry_only | P2.6 |
| AnySearchSkill | A | provider_required, planned_adapter | external_project_registry_only | P2.3 |
| last30days-skill | A | provider_required, future_adapter | external_project_registry_only | P2.3 / P3 |
| skill-prompt-generator | A | benchmark_only, future_adapter | external_project_registry_only | P2.9 |
| MMSkills | A | future_adapter | external_project_registry_only | P2.8 / P3 |
| Jellyfish | A | template_reference, future_adapter | external_project_registry_only | P2.8 / P3 |
| story-flicks | A | template_reference, future_adapter | external_project_registry_only | P2.8 / P3 |
| seedance2-skill | A | provider_required, template_reference, future_adapter | external_project_registry_only | P2.8 / P3 |
| ai-marketing-skills | A | template_reference | external_project_registry_only | P2.7 |
| rtk | A | benchmark_only | external_project_registry_only | P3 |
| OpenDataLoader | A | planned_adapter | external_project_registry_only | P2.6 |
| Marker | A | planned_adapter | external_project_registry_only | P2.6 |
| Surya | A | planned_adapter | external_project_registry_only | P2.6 |
| Unstructured | A | planned_adapter | external_project_registry_only | P2.6 |
| LlamaIndex | A | benchmark_only | external_project_registry_only | P2.5 |
| RAGAS | A | benchmark_only, future_adapter | external_project_registry_only | P2.5 |
| DeepEval | A | benchmark_only, future_adapter | external_project_registry_only | P2.5 |
