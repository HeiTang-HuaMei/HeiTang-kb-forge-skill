...
## P2 User Blackbox Cases

| capability_id | User Path | Required Evidence | Current Status | Gap | Next Gate |
| --- | --- | --- | --- | --- | --- |
| a2a_workgroup | Open Agent, choose common assistant templates, create a test-marked ten-assistant workgroup, run one workgroup task, inspect discussion summary, consensus/conflict report, Event Ledger and Artifact Catalog records, then delete only the current test-marked assistants and generated artifacts. | Ten test-marked assistant instances created from product-facing templates; one completed workgroup run; per-assistant outputs; summary; consensus/conflict report; Event Ledger; Artifact Catalog; restart recovery; delete/tombstone evidence. | `blocked` | P2-1 covers only basic workgroup runtime. P2-4 must add template-seeded ten-agent creation and execution before this case can pass. | `P2-4 A2A >= 10 Agents` |
| official_sample_project_library | Open sample/template library, choose common knowledge base or document template, create a test-marked knowledge base or generated document, inspect output, restart, export/open/delete only the current test-marked object. | Template manifest; created test knowledge base or document; source trace/citation binding where applicable; validation report; Event Ledger; Artifact Catalog; restart recovery; export/open/delete evidence. | `blocked` | P2 must add product template packs; P1 document template registry alone is only the foundation and cannot close this P2 case. | `P2-13 Official Sample Project Library` |
| native_skills_library | Open Skill, choose common Skill template, create a test-marked Skill, bind it to a test knowledge base, validate/export/open/delete and inspect operation history. | Skill template manifest; created test Skill; localized/bound manifest; validation report; operation history; Event Ledger; Artifact Catalog; restart recovery; export/open/delete evidence. | `blocked` | P2 must add native Skill template packs and lifecycle evidence; external Skill import/basic action evidence does not close this P2 case. | `P2-22 Workbench Native Skills Library` |
| office_agent_industrialization | Open document generation or Office workflow, choose a common document template, generate a test document from a test knowledge base, export/open/delete and verify source/citation binding. | Document template manifest; generated test document; source trace/citation binding; export/open/delete evidence; Event Ledger; Artifact Catalog; restart recovery. | `blocked` | P2 must prove template-driven document generation in the Office workflow; P1 document template registry and DOCX basic evidence are not enough. | `P2-25 Office Agent Industrialization` |
