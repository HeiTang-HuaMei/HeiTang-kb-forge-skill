...
def test_version_metadata_is_aligned_to_latest_completed_core_version(tmp_path):
    ...
    assert report["expected_version"] == "4.1.0"
    assert all(item["status"] == "correct" for item in report["records"]), report["records"]
    assert 'version = "4.1.0"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")


def test_version_matrix_is_chronological_and_marks_v4_future():
    text = (ROOT / "docs" / "VERSION_MATRIX.md").read_text(encoding="utf-8")
    order = ["v0.1", "v1.6", "v2.9.0-alpha.1", "v3.12.0-alpha.1", "final-pre-v4.0", "v4.0.0-rc.1", "v4.0.0", "v4.1.0"]
    ...
    assert "| v4.1.0 | Parser/OCR Pluggable Backend Runtime |" in text and "| current | yes |" in text
