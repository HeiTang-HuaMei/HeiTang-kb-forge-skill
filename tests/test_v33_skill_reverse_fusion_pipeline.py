...
def test_pipeline_reports_skill_reverse_fusion_only_when_enabled(tmp_path):
    default_output = _run_pipeline(tmp_path, False)
    enabled_output = _run_pipeline(tmp_path, True)

    default_stage = _stages(default_output)["skill_reverse_fusion"]
    enabled_stage = _stages(enabled_output)["skill_reverse_fusion"]
    assert default_stage["status"] == "skipped"
    assert enabled_stage["status"] == "success"
    assert "skill_fusion_plan.json" in enabled_stage["output_files"]
...
def _run_pipeline(tmp_path, enabled):
    ...
    input_dir.mkdir()
    (input_dir / "lesson.md").write_text("Skill reverse fusion pipeline evidence.", encoding="utf-8")
