...
def test_campaign9_desktop_delivery_status_is_ui_bound_without_release_overclaim():
    ...
    assert status["release_candidate_tag"] == "v4.3.0-rc5"
    ...
    by_capability = {
        item["capability"]: item for item in status["validation_matrix"]
    }
    assert by_capability["full_capability_runtime_chain"]["status"] == "pass"
    assert by_capability["page_button_tab_audit"]["status"] == "pass"
