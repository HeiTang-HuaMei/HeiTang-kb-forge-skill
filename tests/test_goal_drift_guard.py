...
def test_latest_goal_drift_review_records_sequence_lock_without_later_overclaim():
    ...
    assert "closure_checklist_green_verification" in review["task_focus"]
    assert "campaign_1_3_integrated_review_handoff_gate" in review["task_focus"]
    assert "campaign_1_3_review_reports" in review["task_focus"]
    assert "new_conversation_handoff_prompt" in review["task_focus"]
    assert "closure_checklist_green" in review["advanced_in_this_task"]
    assert "campaign_1_3_integrated_review_handoff_gate_passed" in review["advanced_in_this_task"]
    assert "campaign_1_2_3_integrated_review_report_generated" in review["advanced_in_this_task"]
    assert "campaign_1_2_3_external_project_integration_review_generated" in review["advanced_in_this_task"]
    assert "campaign_1_2_3_capability_review_matrix_generated" in review["advanced_in_this_task"]
    assert "new_conversation_handoff_prompt_generated" in review["advanced_in_this_task"]
    ...
    assert "campaign_4_business_implementation" in review["not_advanced_in_this_task"]
    ...
    assert "Campaign 1-3 Integrated Review and New Conversation Handoff Gate passed" in review["next_e2e_gap"]
    assert "Open a new conversation and start Campaign 4 Entry Gate only" in review["next_e2e_gap"]
    assert "Campaign 4 business implementation" in review["next_e2e_gap"]
