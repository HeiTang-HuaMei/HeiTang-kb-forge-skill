...
def test_latest_goal_drift_review_records_sequence_lock_without_later_overclaim():
    ...
    assert "campaign_3_supplement_4_0_acceptance_gate" in review["task_focus"]
    assert "supplement_4_0_boundary_validation" in review["task_focus"]
    assert "product_output_surface_guard_registered" in review["task_focus"]
    assert "next_campaign_3_final_consistency_lock" in review["task_focus"]
    assert "campaign_3_supplement_4_0_acceptance_gate_passed" in review["advanced_in_this_task"]
    assert "supplement_4_0_accepted_for_final_consistency_transition" in review["advanced_in_this_task"]
    assert "next_safe_action_locked_to_campaign_3_final_consistency_gate_only" in review["advanced_in_this_task"]
    assert "campaign_3_final_consistency_gate" in review["not_advanced_in_this_task"]
    assert "campaign_1_3_stage_test_gate" in review["not_advanced_in_this_task"]
    assert "campaign_1_3_integrated_closure_gate" in review["not_advanced_in_this_task"]
    assert "closure_pack" in review["not_advanced_in_this_task"]
    assert "repository_public_surface_cleanup_gate" in review["not_advanced_in_this_task"]
    assert "repository_push" in review["not_advanced_in_this_task"]
    assert "tag" in review["not_advanced_in_this_task"]
    assert "ci_green" in review["not_advanced_in_this_task"]
    ...
    assert "Campaign 3 Supplement 4.0 Acceptance Gate passed" in review["next_e2e_gap"]
    assert "Campaign 3 Final Consistency Gate only" in review["next_e2e_gap"]
    assert "Campaigns 4-9" in review["next_e2e_gap"]
