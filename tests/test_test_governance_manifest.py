...
def test_changed_file_impact_rules_select_development_phase_and_release_gates():
    manifest = load_manifest(MANIFEST_PATH)
    rules = select_impact_rules(["docs/testing/VALIDATION_GATE_MANIFEST.json", "web/workbench/contracts.json"], manifest)

    assert {rule["name"] for rule in rules} >= {"test_governance", "ui_contract"}

    dev_plan = build_validation_plan(["docs/testing/VALIDATION_GATE_MANIFEST.json"], phase="development", manifest=manifest)
    assert {gate["name"] for gate in dev_plan["selected_gates"]} >= {
        "ui_fast_test_governance",
        "ui_fast_version_truth",
    }
    assert dev_plan["release_blocking"] is False

    medium_plan = build_validation_plan(["web/workbench/contracts.json"], phase="phase_closure", manifest=manifest)
    assert [gate["name"] for gate in medium_plan["selected_gates"]] == ["ui_medium_release_truth"]

    release_plan = build_validation_plan(["web/workbench/contracts.json"], phase="release", manifest=manifest)
