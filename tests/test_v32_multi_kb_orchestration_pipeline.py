...
def test_pipeline_reports_multi_kb_orchestration_only_when_enabled(tmp_path):
    default_output = _run_pipeline(tmp_path, False)
    enabled_output = _run_pipeline(tmp_path, True)

    default_stage = _stages(default_output)["multi_kb_orchestration"]
    enabled_stage = _stages(enabled_output)["multi_kb_orchestration"]
...
def _run_pipeline(tmp_path, enabled):
    ...
    input_dir.mkdir()
    (input_dir / "lesson.md").write_text("Multi KB pipeline evidence.", encoding="utf-8")
