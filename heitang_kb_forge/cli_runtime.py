...
from heitang_kb_forge.skill import (
    SKILL_PACKAGE_FILES,
...
def generate_skill_command(
    ...
    on_demand: bool = typer.Option(True, "--on-demand/--no-on-demand"),
    token_budget: int = typer.Option(4000, "--token-budget"),
    update_existing: Path | None = typer.Option(None, "--update-existing", exists=True, file_okay=False, dir_okay=True),
    preserve_manual_edits: bool = typer.Option(False, "--preserve-manual-edits"),
...
@app.command("book-to-skill")
def book_to_skill_command(
    input: str = typer.Option("", "--input", help="Source file, folder, or glob. Use with --package omitted."),
    package: Path | None = typer.Option(None, "--package", exists=True, file_okay=False, dir_okay=True, readable=True),
    output: Path = typer.Option(..., "--output", "-o"),
    skill_name: str = typer.Option("Structured Knowledge Skill", "--skill-name"),
    target: str = typer.Option("generic", "--target"),
    language: str = typer.Option("auto", "--language"),
    on_demand: bool = typer.Option(True, "--on-demand/--no-on-demand"),
    token_budget: int = typer.Option(4000, "--token-budget"),
    ...
    """Generate a structured on-demand Skill package from sources or an existing KB package."""
    if package is None and not input:
        typer.echo("--input or --package is required", err=True)
        raise typer.Exit(2)
    if target not in {"claude_code", "codex", "openclaw", "generic"}:
        typer.echo("--target must be one of: claude_code, codex, openclaw, generic", err=True)
        raise typer.Exit(2)
    output.mkdir(parents=True, exist_ok=True)
    package_path = package
    if package_path is None:
        inputs = collect_book_to_skill_inputs([input])
        if not inputs:
            typer.echo(f"No supported input files found for --input {input}", err=True)
            raise typer.Exit(2)
        package_path = output / "knowledge_package"
        _build_package(
            inputs[0].parent,
            package_path,
            domain,
            ...
            overlap_chars,
            source_files=inputs,
            retrieval_index_options=RetrievalIndexOptions(True),
            contract_options=ContractOptions("v2", True, False),
        )
    assert_trusted_for_export(package_path, allow_untrusted=allow_untrusted)
    skill_output = output / "skill_package" if package is None else output
    result = generate_skill_package(
        package_path,
        skill_output,
        skill_name,
        "structured_book_skill",
        generated_by="book_to_skill",
        target=target,
        language=language,
        on_demand=on_demand,
        token_budget=token_budget,
...
def export_skill_pack_command(
    suite: Path = typer.Option(
        ..., "--suite", exists=True, file_okay=False, dir_okay=True, readable=True
    ),
    output: Path = typer.Option(..., "--out", "-o"),
) -> None:
    """Export a bounded local-first Skill Pack from a built Skill Suite."""
    result = export_skill_pack(suite, output)
...
def validate_skill_package_command(
    skill: Path = typer.Option(..., "--skill", exists=True, file_okay=False, dir_okay=True, readable=True),
    output: Path = typer.Option(..., "--output", "-o"),
) -> None:
    """Validate a structured book-to-skill package."""
    result = validate_structured_skill_package(skill, output)
    typer.echo(f"Built structured Skill validation at {output}")
    typer.echo(f"Status: {result['status']} | Release ready: {result['release_ready']}")


@app.command("diff-skill-package")
def diff_skill_package_command(
    old_skill: Path = typer.Option(..., "--old-skill", exists=True, file_okay=False, dir_okay=True, readable=True),
    new_skill: Path = typer.Option(..., "--new-skill", exists=True, file_okay=False, dir_okay=True, readable=True),
    output: Path = typer.Option(..., "--output", "-o"),
) -> None:
    """Diff two structured Skill packages and report stale/regeneration status."""
...
def _run_config(config_data: ForgeConfig) -> ConfigRunResult:
    ...
    if config_data.task == "build":
        ...
        _run_structured_skill_generation_config(config_data, config_data.output)
...
def _run_structured_skill_generation_config(config_data: ForgeConfig, output: Path) -> None:
    if not config_data.skill_generation.enabled:
        return
    target = config_data.skill_generation.target
    if target not in {"claude_code", "codex", "openclaw", "generic"}:
        raise typer.BadParameter("skill_generation.target must be one of: claude_code, codex, openclaw, generic")
    if config_data.skill_generation.output_structure != "structured_book_to_skill":
        raise typer.BadParameter("skill_generation.output_structure must be structured_book_to_skill")
    assert_trusted_for_export(output, allow_untrusted=config_data.parser_backend.allow_untrusted)
    skill_output = output / "structured_skill_package"
    generate_skill_package(
        output,
        skill_output,
        config_data.skill.name,
        config_data.skill.type,
        generated_by="config_skill_generation",
        target=target,
        language=config_data.skill_generation.language,
        on_demand=config_data.skill_generation.on_demand_loading,
        token_budget=config_data.skill_generation.token_budget,
        update_existing=config_data.skill_generation.update_existing,
        preserve_manual_edits=config_data.skill_generation.preserve_manual_edits,
    )
    validate_structured_skill_package(skill_output, output / "structured_skill_validation")
