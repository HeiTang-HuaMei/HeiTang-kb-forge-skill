...
from heitang_kb_forge.product_hardening import V312_PRODUCT_HARDENING_OUTPUT_FILES
...
def make_pipeline_report(*, config_file: Path, config: ForgeConfig, output: Path) -> tuple[PipelineManifest, str]:
    stages = [
        ...
        _stage("product_hardening_release_readiness", config.product_hardening.enabled, config.product_hardening.output or output, V312_PRODUCT_HARDENING_OUTPUT_FILES, config.task),
