...
def make_p1_workbench_dry_run(action_id: str) -> dict:
    ...
    return {
        ...
        "trace_id": f"dry_run_{action_id}",
        "product_status": "blocked" if action["status"] == "blocked" else "queued",
        "command_kind": action["command_kind"],
        "blocked_reason": action["blocked_reason"],
        "user_reason": action["blocked_reason"] or "Dry-run only; no local Core process was started.",
        "retry_suggestion": "Run the allowlisted execute action only after reviewing inputs and output path.",
