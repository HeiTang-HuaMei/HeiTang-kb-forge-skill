        ...
        for extraction_type in EXTRACTION_TYPES:
            ...
            try:
                ...
                if response_payload is None:
                    response = provider.generate_json(_prompt(chunk, extraction_type), extraction_type)
                ...
                result.outputs[extraction_type].extend(
                    _records_from_payload(response_payload, chunk, extraction_type, cache_key)
                )
...
def _prompt(chunk: Chunk, extraction_type: str) -> str:
...
def _records_from_payload(response_payload: dict, chunk: Chunk, extraction_type: str, cache_key: str) -> list[dict]:
