...
状态：在 P1 Final Gate、External Project Registry、S/A Contract Inclusion、rc.1 acceptance 与 release hardening 之后，进入 v4.0.0 stable release。
...
- Pre-v4 External Project Registry 与 S/A Contract Inclusion 已完成。
...
- `ready_for_v4_rc=true`；rc.1 acceptance 与 hardening evidence 已提升到 stable `v4.0.0` release line。
...
- P1 local Workbench gate、rc.1 acceptance 与 release hardening evidence 已完成，可支撑 stable `v4.0.0`。
...
- Docling、PaddleOCR 和 Unstructured 只作为 opt-in 本地 parser/OCR runtime adapters 实现；它们依赖本地安装，不打包进 Core，不是默认 parser 路径，也不是 UI 可执行外部项目。
