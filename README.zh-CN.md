...
当前 UI package 版本：`4.2.0`

当前 stable release：`v4.2.0` P2.2 Knowledge-to-Methodology-to-Skill-Suite Industrial Baseline

发布状态：在 v4.1.1 P2.2 Entry Gate / Test Governance Stable Baseline 与 v4.1.0 Parser/OCR Workbench 同步之后，v4.2.0 UI industrial workflow 已进入 release closure。`v4.0.0`、`v4.1.0`、`v4.1.1` tag 保持不变；v4.1.0 parser/OCR fixture 仍是 static Workbench 消费的历史 Core evidence。
...
- P2.2 Skill Factory industrial workflow：Knowledge Package、Evidence、Methodology、Candidates、Hierarchy、Skill Suite、Reports、Export evidence surfaces
...
v4.2.0 中，UI package 完成 P2.2 Skill Factory workflow 的 static evidence surface：Knowledge Package -> Evidence -> Methodology -> Candidates -> Hierarchy -> Skill Suite -> Reports -> Export。它展示 methodology、candidate、dependency、routing、validation、diff、installability、governance 和 export evidence，但 Web build 不执行本地 CLI。

v4.1.1 中，UI package 增加 test framework governance，并保持为 P2.2 Entry Gate / Test Governance Stable Baseline。parser/OCR fixture 仍是 v4.1.0 历史 Core evidence，不改写成新的 runtime execution 声明。
