...
try {
  ...
  $pages = @(
    ...
    @{ page = "文档生成"; key = "5"; ry = 0.52; buttons = @("生成 Markdown", "导出 Markdown", "DOCX/PDF/PPTX gate", "删除最近记录") },
    @{ page = "技能生成"; key = "6"; ry = 0.60; buttons = @("生成 Skill", "外部 Skill 导入") },
    @{ page = "我的助手"; key = "7"; ry = 0.68; buttons = @("创建助手", "发送对话", "启动工作小组", "清空对话") },
    @{ page = "设置"; key = "8"; ry = 0.76; buttons = @("保存设置", "测试连接", "重置/回滚", "创建配置档", "切换配置档") },
    @{ page = "知识库验证（二级）"; key = "4"; ry = 0.44; buttons = @("知识库内部验证", "保存报告", "来源/证据查看") },
    @{ page = "全部成果（二级）"; key = "9"; ry = 0.84; buttons = @("成果打开", "成果删除/清理") },
    @{ page = "操作记录（二级）"; key = "0"; ry = 0.89; buttons = @("查看记录", "导出操作记录", "清理记录 gate") }
  )
  ...
  $actionMap = @(
    ...
    @{ page = "知识库"; button = "验证知识库"; event = "query"; flag = "retrieval"; expected = "检索/验证知识库并生成可追溯结果" },
    ...
    @{ page = "我的助手"; button = "创建助手"; event = "generate_agent"; flag = "agent"; expected = "创建助手产物" },
    @{ page = "我的助手"; button = "发送对话"; event = "agent_dialogue"; flag = "agent_dialogue"; expected = "执行单助手对话并生成记录" },
    @{ page = "全部成果（二级）"; button = "成果打开"; event = "artifact_records"; flag = "markdown"; expected = "全部成果能映射真实产物路径" },
    @{ page = "操作记录（二级）"; button = "导出操作记录"; event = "audit_report"; flag = "audit_report"; expected = "导出真实操作记录报告" }
  )
  ...
  $gatedButtons = @(
    ...
    @{ page = "我的助手"; button = "启动工作小组"; capability = "工作小组依赖项"; expected = "未配置依赖项不得假成功" },
    @{ page = "设置"; button = "测试连接"; capability = "连接配置"; expected = "未配置时需要设置/本地模式" },
